(function () {
    ...
    window.CanvasSlideshow = function (options) {
        ...
        this.initPixi = function () {
            ...
            document.getElementById("canvas_area").appendChild(renderer.view);
        ...
        this.moveSlider = function (newIndex) {
            ...
            baseTimeline
                .to(displacementFilter.scale, 0.5, {
                    ...
                    y: options.displaceScale[20]
                }, 'firstImageFadeOut')
                .to(slideImages[that.currentIndex], 0.5, {
                    alpha: 0
                }, 'firstImageFadeOut+=0.25')
                .to(slideImages[newIndex], 0.5, {
                    alpha: 1
                }, 'firstImageFadeOut+=0.25')
                .to(displacementFilter.scale, 0.5, {
                    x: options.displaceScaleTo[0],
                    y: options.displaceScaleTo[20]
                }, 'firstImageFadeOut+=0.5');
        ...
        for (var i = 0; i < nav.length; i++) {
            ...
            navItem.onclick = function (event) {
                ...
                if (this.getAttribute('data-nav') === 'zero') {
                    that.moveSlider(0);
                } else if (this.getAttribute('data-nav') === 'one') {
                    that.moveSlider(1);
                } else if (this.getAttribute('data-nav') === 'two') {
                    that.moveSlider(2);
                } else if (this.getAttribute('data-nav') === 'three') {
                    that.moveSlider(3);
                } else if (this.getAttribute('data-nav') === 'four') {
                    that.moveSlider(4);
                }
            ...
            if (options.interactionEvent === 'hover' || options.interactionEvent === 'both') {
                ...
                slidesContainer.pointerover = function (mouseData) {
                    ...
                    TweenMax.to(displacementFilter.scale, 1, {
                        x: "+=" + Math.sin(mouseX) * 25 + "",
                        y: "+=" + Math.cos(mouseY) * 25 + ""
                    });
                    rotateSpite();
                };
